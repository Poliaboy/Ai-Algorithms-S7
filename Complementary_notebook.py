...
def show_map2(graph_data, node_colors=None,tour=None):
    G = nx.Graph(graph_data['graph_dict'])
    node_colors = node_colors or graph_data['node_colors']
    node_positions = graph_data['node_positions']
    node_label_pos = graph_data['node_label_positions']
    ...
    nx.draw(G, pos={k: node_positions[k] for k in G.nodes()},
            node_color=[node_colors[node] for node in G.nodes()], linewidths=0.3, edgecolors='k')
    # Highlight the edges of the tour if it is provided
    if tour:
        tour_edges = [(tour[i], tour[i + 1]) for i in range(len(tour) - 1)]
        tour_edges.append((tour[0], tour[-1]))
        nx.draw_networkx_edges(G, pos=node_positions, edgelist=tour_edges, edge_color='g', width=4)
    ...
    # add edge lables to the graph
    #x.draw_networkx_edge_labels(G, pos=node_positions, edge_labels=edge_weights, font_size=14)


    # show the plot. No need to use in notebooks. nx.draw will show the graph itself.
    plt.show()
    
def show_map3(graph_data, node_colors=None,tour=None):
    G = nx.Graph(graph_data['graph_dict'])
    node_colors = node_colors or graph_data['node_colors']
    node_positions = graph_data['node_positions']
    node_label_pos = graph_data['node_label_positions']
    ...
    plt.figure(figsize=(18, 13))
    # draw the graph (both nodes and edges) with locations from romania_locations
    nx.draw_networkx_nodes(G, pos={k: node_positions[k] for k in G.nodes()},
